...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (response.ok && result.status === 'success') {
      submitMessage.value = 'Your quotation request has been successfully submitted! Our sales team will contact you shortly.'
      isError.value = false
  ...
  <div class="modal-overlay" :class="{ 'is-active': isRequestModalOpen }" @click="closeModal">
    <div class="modal-content" @click.stop>
      ...
      <div class="modal-body">
        <h2 class="modal-title">Request a Quotation</h2>
        <p class="modal-subtitle">Provide details of the spare parts you need, and our experts will get back to you with competitive pricing.</p>
        
        <form @submit.prevent="handleSubmit" class="modal-form">
          <div class="form-group">
            <label class="form-label" for="quote-name">Company / Contact Name *</label>
            <input 
              v-model="form.name"
              id="quote-name"
              type="text" 
          ...
          <div class="form-group">
            <label class="form-label" for="quote-msg">List of Spare Parts & Quantities *</label>
            <textarea 
              v-model="form.message"
              id="quote-msg"
              class="form-textarea" 
              rows="4" 
              placeholder="Example: Toyota Camry 2018 Front Brake Pads - 2 sets, Engine Oil Filter - 5 pcs..."
              required
            ></textarea>
          </div>
          ...
          <button type="submit" class="btn btn-primary submit-btn" :disabled="isSubmitting">
            <span v-if="!isSubmitting"><Send size="16" /> Send Inquiry</span>
